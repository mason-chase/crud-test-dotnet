...
public class UpdateCustomerHandlerTests
{
    ...
    private UpdateCustomerHandler Sut => new UpdateCustomerHandler(_repository, _eventStore);

    private readonly Customer _aggregate;
    private readonly UpdateCustomerCommand _command;
    ...
    public UpdateCustomerHandlerTests()
    {
        ...
        _command = new UpdateCustomerCommand(new CustomerId(1), firstName, lastName, dateOfBirth, phoneNumber, email, bankAccountNumber);
    }
    ...
    public async Task Handle_should_throw_CustomerAlreadyExistsException_when_Email_is_already_taken_by_another_Customer()
    {
        ...
        _repository
            .AnyAsync(Arg.Is<Expression<Func<Customer, bool>>>(s =>
                LambdaCompare.Eq(s, e => e.Id != _command.CustomerId.Value && e.Email == _command.Email.Value)), Arg.Any<CancellationToken>())
            .Returns(true);
    ...
    public async Task Handle_should_throw_CustomerAlreadyExistsException_when_FirstName_LastName_and_DateOfBirth_is_already_taken_by_another_Customer()
    {
        ...
        _repository
            .AnyAsync(Arg.Is<Expression<Func<Customer, bool>>>(s =>
                LambdaCompare.Eq(s, e => e.Id != _command.CustomerId.Value && e.FirstName == _command.FirstName.Value && e.LastName == _command.LastName.Value && e.DateOfBirth == _command.DateOfBirth.ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc))), Arg.Any<CancellationToken>())
            .Returns(true);
