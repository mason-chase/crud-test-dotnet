   ...
   /* public override async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        foreach (var contextActionArgument in context.ActionArguments.Values)
        {
            ...
            if (viewModelValidator is IValidator validator)
            {
                var validationResult =
                    await validator.ValidateAsync(new ValidationContext<object>(contextActionArgument));
        ...
        if (!modelState.IsValid)
        {
            ...
            if (model != null)
            {
                ...
                var apiResult = new ApiResult<IDictionary<string, string[]>>(false, ApiResultStatusCode.BadRequest,
